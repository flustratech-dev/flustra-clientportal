    ...
    @else
        @foreach($contracts as $k)
            <div class="erp-card" x-data="{ showModal: false }">
                    ...
                    <form action="{{ route('vendor.kontrak.acknowledge', $k['id']) }}" method="POST"
                        ...
                        <label class="flex gap-2 items-start cursor-pointer">
                            ...
                            <span class="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed">
                                Saya telah membaca isi naskah perjanjian pengadaan barang/jasa ini dan menyetujuinya secara sah mewakili perusahaan saya.
                ...
                {{-- Modal Naskah Perjanjian Vendor Standar Enterprise --}}
                <div x-show="showModal" x-cloak
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden"
                        ...
                        <div class="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between no-print bg-slate-50 dark:bg-slate-800/50">
                            <div class="flex items-center gap-2">
                                <span class="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider">Draf Naskah Perjanjian Kerja Sama Vendor</span>
                            </div>
                            <div class="flex items-center gap-2">
                        ...
                        <div class="p-6 sm:p-8 overflow-y-auto space-y-6 text-slate-800 dark:text-slate-200 text-xs leading-relaxed font-sans bg-white dark:bg-slate-900">
                            ...
                            <div class="flex items-center justify-between pb-4 border-b-2 border-slate-900 dark:border-slate-100">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-[10px] text-slate-600 dark:text-slate-400">Gedung Flustra Tower Lt. 8, Jakarta &bull; procurement@flustra.id &bull; www.flustra.id</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <span class="text-[10px] font-mono font-bold px-2 py-0.5 border border-slate-900 dark:border-slate-100 rounded">DOKUMEN PENGADAAN RESMI</span>
                                    <p class="text-[9px] text-slate-500 mt-1 font-mono">PKS/FLS-VND/{{ date('Y') }}/{{ str_pad($k['id'], 4, '0', STR_PAD_LEFT) }}</p>
                                </div>
                            </div>
                            ...
                            <div class="text-center py-2">
                                <h3 class="text-sm font-black text-slate-900 dark:text-white uppercase tracking-wide">
                                    PERJANJIAN KERJA SAMA PENGADAAN BARANG & JASA (VENDOR AGREEMENT)
                                </h3>
                            ...
                            <div class="space-y-2 text-[11px] text-justify">
                                ...
                                <div class="pl-4 space-y-1.5 border-l-2 border-slate-200 dark:border-slate-700">
                                    ...
                                    <p>
                                        <strong>2. {{ auth()->user()->name }}</strong> (selaku perwakilan resmi dari Mitra Vendor / Supplier yang terdaftar sah pada sistem Flustra Vendor Portal), berkedudukan di alamat terdaftar (selanjutnya disebut sebagai <strong>"PIHAK KEDUA"</strong>).
                                    </p>
                                </div>
                                <p>
                            ...
                            <div class="space-y-4 pt-2">
                                <div>
                                    <h4 class="font-bold text-slate-900 dark:text-white text-[11px]">PASAL 1: RUANG LINGKUP PENGADAAN</h4>
                                    <p class="mt-1 text-slate-600 dark:text-slate-400">
                                        PIHAK KEDUA setuju untuk memasok, memproduksi, dan/atau menyediakan barang dan jasa kepada PIHAK PERTAMA sesuai dengan judul kontrak: <em>"{{ $k['title'] }}"</em> serta Purchase Order (PO) yang diterbitkan secara resmi.
                                    </p>
                                </div>
                                ...
                                <div>
                                    <h4 class="font-bold text-slate-900 dark:text-white text-[11px]">PASAL 3: HARGA, PENAGIHAN & SYARAT PEMBAYARAN</h4>
                                    <p class="mt-1 text-slate-600 dark:text-slate-400">
                                        @if($k['value'] > 0)
                                            Total nilai plafon kerja sama pengadaan ini adalah sebesar <strong>Rp {{ number_format($k['value'], 0, ',', '.') }}</strong>. Pembayaran dilakukan melalui transfer bank resmi dalam tempo yang ditentukan setelah PIHAK KEDUA menyerahkan Faktur Asli, Surat Jalan/GRN, dan Faktur Pajak lengkap.
                                        @else
                                            Harga dan volume pengadaan mengacu pada setiap Purchase Order (PO) yang diterbitkan oleh PIHAK PERTAMA dan dikonfirmasi oleh PIHAK KEDUA.
                                        @endif
                                    </p>
                                </div>

                                <div>
                                    <h4 class="font-bold text-slate-900 dark:text-white text-[11px]">PASAL 4: STANDAR KUALITAS, JAMINAN (WARRANTY) & RETUR</h4>
                                    <p class="mt-1 text-slate-600 dark:text-slate-400">
                                        PIHAK KEDUA menjamin bahwa seluruh barang/jasa yang diserahkan dalam kondisi baru, berkualitas prima, bebas dari cacat tersembunyi, dan sesuai spesifikasi. Barang yang tidak memenuhi standar mutu berhak diretur atau dipotongkan melalui Nota Debet (Debit Note).
                                    </p>
                                </div>
                                ...
                                <table class="w-full text-center text-xs">
                                    <tr>
                                        <td class="w-1/2 align-top pb-12">
                                            ...
                                            <p class="font-bold text-slate-800 dark:text-white underline">DIVISI PENGADAAN (PROCUREMENT)</p>
                                        </td>
                                        <td class="w-1/2 align-top pb-12">
                                            <p class="font-bold text-slate-900 dark:text-white">PIHAK KEDUA</p>
                                            <p class="text-[10px] text-slate-500">MITRA VENDOR / SUPPLIER</p>
                                            <div class="my-6">
                                                @if($k['customer_ack_at'])
